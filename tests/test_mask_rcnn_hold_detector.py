...
class Instances:
    def __init__(self, masks, scores, classes):
        self.pred_masks, self.scores, self.pred_classes = torch.as_tensor(masks), torch.as_tensor(scores), torch.as_tensor(classes)
    def to(self, device):
        assert device == "cpu"
        return self
...
class Model:
    ...
    def __call__(self, inputs): return [{"instances": item} for item in self.instances]
...
def test_converts_source_coordinate_masks_and_preserves_order():
    ...
    second = np.zeros((12, 16), dtype=bool); second[1:5, 1:5] = True
...
def test_empty_detections_and_volume_filtering():
    detector = MaskRCNNHoldDetector(device="cpu")
    detector.model = Model([Instances(np.empty((0, 12, 16), bool), [], []), Instances([mask(), mask()], [.9, .8], [0, 1])])
    result = detector.get_holds([Image.new("RGB", (16, 12)), Image.new("RGB", (16, 12))])
    assert result[0] == [] and [hold.attributes["class_name"] for hold in result[1]] == ["hold"]
    detector.include_volumes = True
    assert len(detector.get_holds([Image.new("RGB", (16, 12)), Image.new("RGB", (16, 12))])[1]) == 2

def test_invalid_input_and_overlay_alignment():
    detector = MaskRCNNHoldDetector(device="cpu")
    with pytest.raises(InvalidImageError): detector.get_holds(["bad"])
    with pytest.raises(BatchAlignmentError): detector.mark_holds([Image.new("RGB", (1, 1))], [])
