...
class _Provider:
    def __init__(
        ...
        derive: Callable[[ImageRecord], ImageRecord | None],
    ) -> None:
    ...
    def iter_split(self, split: str) -> Iterable[ImageRecord]:
        if split not in self._spec.splits:
        ...
        for record in self._records:
            if record.split == split:
                ...
                if derived is not None:
                    yield replace(derived, image=derived.image.copy())
...
class DataPreprocessingPipeline:
    ...
    def _split(self, record: ImageRecord, spec: DatasetSpec) -> str:
        if record.split is not None:
            if record.split not in spec.splits:
                raise ValueError(
                    f"Record '{record.image_id}' has split '{record.split}', absent from DatasetSpec."
                )
            return record.split
        weights = [self._WEIGHTS[split] for split in spec.splits]
        total = sum(weights)
        value = (
            int.from_bytes(
                sha256(f"{spec.seed}:{record.source_id}".encode()).digest()[:8], "big"
            )
            / 2**64
        )
        boundary = 0.0
    ...
    def _derived(self, record: ImageRecord, spec: DatasetSpec) -> ImageRecord | None:
        plan = spec.augmentation_plan
        if plan is None or not plan.augmentations:
            return None
        ...
        return ImageRecord(
            image_id=f"{record.image_id}:aug:{fingerprint}",
            image=image,
            ...
            condition="augmented",
            augmentation_metadata=self._recipe_metadata(spec),
        )
    ...
    def build(
        ...
        if not isinstance(spec, DatasetSpec):
            raise TypeError("spec must be DatasetSpec.")
        records = tuple(sources)
        if any(not isinstance(record, ImageRecord) for record in records):
            raise TypeError("sources must contain ImageRecord values.")
        clean = tuple(
            replace(
                record,
                image=record.image.copy(),
                split=self._split(record, spec),
                source_id=record.source_id,
                condition="clean",
