...
class ChalkAugmentationParams:
    intensity: float = 0.5
    def __post_init__(self) -> None: _strength(self.intensity, "intensity", 0, 1)
...
class ColorAugmentationParams:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.color, RGBColor): raise TypeError("color must be RGBColor.")
        _strength(self.intensity, "intensity", 0, 1)

@dataclass(frozen=True)
class LightingAugmentationParams:
    intensity: float = 0.0
    def __post_init__(self) -> None: _strength(self.intensity, "intensity", -1, 1)
...
class ChalkAugmentation:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.params, ChalkAugmentationParams): raise TypeError("params must be ChalkAugmentationParams.")
...
class ColorAugmentation:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "polygons", _polygons(self.polygons))
        if not isinstance(self.params, ColorAugmentationParams): raise TypeError("params must be ColorAugmentationParams.")
...
class LightingAugmentation:
    ...
    name: str = "lighting"
    def __post_init__(self) -> None:
        if not isinstance(self.params, LightingAugmentationParams): raise TypeError("params must be LightingAugmentationParams.")
...
class AugmentationPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if any(not isinstance(recipe, (ChalkAugmentation, ColorAugmentation, LightingAugmentation)) for recipe in recipes):
            raise TypeError("augmentations must contain supported augmentation recipes.")
