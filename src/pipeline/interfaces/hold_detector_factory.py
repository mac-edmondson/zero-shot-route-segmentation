...
class UnknownHoldDetectorError(ValueError): pass
class InvalidHoldDetectorConfigError(ValueError): pass
...
class HoldDetectorFactory:
    ...
    def create(self, detector_method: str, config: Mapping[str, Any] | None = None) -> HoldDetector:
        """Create a detector from its method ID and configuration."""
        if detector_method not in self._constructors:
            raise UnknownHoldDetectorError(f"Unknown hold detector '{detector_method}'. Available methods: {', '.join(self.available_methods())}.")
        if config is not None and not isinstance(config, Mapping): raise InvalidHoldDetectorConfigError("Config must be a mapping.")
        try: return self._constructors[detector_method](dict(config or {}))
        except InvalidHoldDetectorConfigError: raise
        except (TypeError, ValueError) as error: raise InvalidHoldDetectorConfigError(str(error)) from error
