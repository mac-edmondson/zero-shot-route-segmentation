...
from .metrics import match_holds, polygon_iou
...
MATRIX_ROUTE_SPECS = (
    ...
    *(ComponentSpec("DINO Clustering", {"clustering_method": method, "color_weight": weight})
...
def _relabel_predictions(record: ImageRecord, predictions: Sequence[Hold], threshold: float = 0.5) -> tuple[Hold, ...]:
    annotations = tuple(a for a in record.annotations if a.attributes.get("hold_type") != "volume")
    matches = match_holds(predictions, annotations, threshold=threshold, image_size=record.image.size)
    route_ids = {m.prediction_index: annotations[m.annotation_index].attributes.get("route_id") for m in matches}
    return tuple(Hold(p.polygon, {**p.attributes, **({"route_id": route_ids[i]} if isinstance(route_ids.get(i), int) else {})}) for i, p in enumerate(predictions))
...
class _MappedRouteDiscriminator:
    def __init__(self, discriminator: Any, threshold: float = 0.5):
        ...
        self.implementation_id = getattr(discriminator, "implementation_id", type(discriminator).__name__)
        ...
        for image, source, routes in zip(images, holds, outputs, strict=True):
            ...
            for route_index, route in enumerate(routes):
                ...
                for output in route.holds:
                    candidates = ((polygon_iou(output.polygon, hold.polygon, image.size), i, hold)
                                  for i, hold in enumerate(source) if i not in used)
                    iou, index, hold = max(candidates, default=(0.0, -1, None))
...
def _slug(spec: ComponentSpec) -> str:
    ...
    return "-".join((spec.name.lower().replace(" ", "_"), config)).strip("-").replace(".", "_")


def run_evaluation_matrix(
    records: Sequence[ImageRecord],
    *,
    detectors: Sequence[ComponentSpec] = tuple(ComponentSpec(name, {}) for name in DEFAULT_DETECTORS),
    ...
    manifest = {"schema_version": 1, "run_id": run_id, "record_count": len(records), "combinations": [], "errors": []}
    for detector_spec in detectors:
        try:
            ...
            for record in records:
                ...
                outputs = _quiet_call(detector.get_holds, [record.image])
                if len(outputs) != 1:
                    raise ValueError("detector must return one hold sequence per image")
                predictions[record.image_id] = tuple(outputs[0])
                timings[record.image_id] = perf_counter() - started
            detector_report = _quiet_call(
                evaluate_hold_detector,
                detector, records,
                run_id=f"{run_id}:{detector_spec.name}",
                predictions_by_image=predictions, inference_seconds_by_image=timings,
            ).to_dict()
            route_records = [ImageRecord(r.image_id, r.image, _relabel_predictions(r, predictions[r.image_id]), r.split, r.source_id, r.parent_image_id, r.condition, r.augmentation_metadata) for r in records]
        except Exception as error:  # noqa: BLE001 - preserve matrix progress
            manifest["errors"].append({"component_type": "detector", "name": detector_spec.name, "error": f"{type(error).__name__}: {error}"})
            continue
        for route_spec in route_discriminators:
            name = f"{detector_spec.name}__{_slug(route_spec)}"
            path = root / f"{_slug(ComponentSpec(detector_spec.name, {}))}__{_slug(route_spec)}.json"
            try:
                discriminator = route_discriminator_factory(route_spec.name, route_spec.config)
                route_report = _quiet_call(
                    evaluate_route_discriminator,
                    _MappedRouteDiscriminator(discriminator), route_records,
                    run_id=f"{run_id}:{name}",
                ).to_dict()
                payload = {
                    "schema_version": 1, "run_id": f"{run_id}:{name}",
                    "detector": detector_report, "route_discriminator": route_report,
                    "metadata": {"protocol": "detector_predictions_then_route_grouping", "match_threshold": 0.5},
                }
                path.write_text(json.dumps(payload, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
                manifest["combinations"].append({"name": name, "path": str(path), "status": "completed", "detector": detector_spec.name, "route_discriminator": route_spec.name, "configuration": route_spec.config})
            except Exception as error:  # noqa: BLE001 - preserve matrix progress
                manifest["errors"].append({"component_type": "route_discriminator", "name": name, "error": f"{type(error).__name__}: {error}"})
                manifest["combinations"].append({"name": name, "path": str(path), "status": "error"})
...
def run_independent_evaluations(
    ...
    *,
    ...
    detector_result = {"schema_version": 1, "run_id": run_id, "record_count": len(records), "detectors": [], "errors": []}
    route_result = {"schema_version": 1, "run_id": run_id, "record_count": len(records), "route_discriminators": [], "errors": []}
    for spec in detectors:
        try:
            report = _quiet_call(
                evaluate_hold_detector,
                hold_detector_factory(spec.name, spec.config), records,
            ...
            detector_result["errors"].append({"name": spec.name, "error": f"{type(error).__name__}: {error}"})
    for spec in route_discriminators:
        try:
            report = _quiet_call(
                evaluate_route_discriminator,
                route_discriminator_factory(spec.name, spec.config), records,
                run_id=f"{run_id}:{spec.name}",
            )
            route_result["route_discriminators"].append(report.to_dict())
        except Exception as error:  # noqa: BLE001 - preserve independent progress
            route_result["errors"].append({"name": spec.name, "configuration": spec.config, "error": f"{type(error).__name__}: {error}"})
    detector_path = root / "detectors.json"
    route_path = root / "route_discriminators.json"
    detector_path.write_text(json.dumps(detector_result, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
    route_path.write_text(json.dumps(route_result, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", default="results/evaluation_matrix/manifest.json", type=Path)
    parser.add_argument("--split", default="test")
    parser.add_argument("--run-id")
    parser.add_argument("--independent", action="store_true", help="Run detector-only and ground-truth-route evaluations.")
    parser.add_argument("--output-dir", type=Path, help="Output directory for independent reports.")
    parser.add_argument(
        "--detector",
        action="append",
        dest="detectors",
...
def main(argv: list[str] | None = None) -> int:
    ...
    detectors = tuple(ComponentSpec(name, {}) for name in (args.detectors or DEFAULT_DETECTORS))
    routes = MATRIX_ROUTE_SPECS if not args.route_discriminators else tuple(ComponentSpec(name, {}) for name in args.route_discriminators)
    if args.independent:
        result = run_independent_evaluations(records, detectors=detectors, route_discriminators=routes, output_dir=args.output_dir or Path("results/independent"), run_id=args.run_id)
        print(f"Saved {len(result['detectors']['detectors'])} detector and {len(result['route_discriminators']['route_discriminators'])} route reports.")
    else:
        manifest = run_evaluation_matrix(records, detectors=detectors, route_discriminators=routes, output_dir=args.output.parent, run_id=args.run_id)
        print(f"Saved {len(manifest['combinations'])} matrix combinations to {args.output.parent} ({len(manifest['errors'])} errors).")
    return 0
